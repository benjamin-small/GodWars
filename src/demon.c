...
void do_angelarmour(CHAR_DATA *ch, char *argument) {
    ...
    if(!str_cmp(arg1, "yellow")) {
        obj->specpower = ARM_YELLOW;
        ch->pcdata->quest -= 100;
        obj->points += 100;
    }
    if(!str_cmp(arg1, "blue")) {
        obj->specpower = ARM_BLUE;
        ch->pcdata->quest -= 250;
        obj->points += 250;
    }
    if(!str_cmp(arg1, "silver")) {
        obj->specpower = ARM_SILVER;
        ch->pcdata->quest -= 750;
        obj->points += 750;
    }
    if(!str_cmp(arg1, "gold")) {
        obj->specpower = ARM_GOLD;
        ch->pcdata->quest -= 1500;
        obj->points += 1500;
    }
