...
void improve_spl(CHAR_DATA *ch, int dtype, int sn) {
    ...
    if(ch->spl[dtype] >= 200 && !IS_MAGE(ch)) {
        return;
    }
    if(ch->spl[dtype] >= 240) {
        return;
    ...
    for(vch = char_list; vch != NULL; vch = vch_next) {
        ...
        if(vch->in_room == ch->in_room) {
            if(vch != ch && (IS_NPC(ch) ? !IS_NPC(vch) : IS_NPC(vch))) {
                ...
                hp = vch->hit;
                if(IS_IMMUNE(vch, IMM_LIGHTNING)) {
                    vch->hit += dam;
                }
                damage(ch, vch, dam, sn);
                if(vch == NULL || vch->position == POS_DEAD) {
                    continue;
                }
                if(IS_IMMUNE(vch, IMM_LIGHTNING)) {
                    vch->hit = hp;
...
void spell_poison(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if(!IS_NPC(victim) && IS_VAMPIRE(victim)) {
        return;
              ...
              victim->pcdata->disc[TOTEM_SPIDER] > 2) {
        return;
    }
    if(!IS_NPC(victim) && victim->pcdata->atm >= number_percent()) {
        return;
