        ...
        for(d = descriptor_list; d != NULL; d = d->next) {
            ...
            if(d->connected == CON_PLAYING
                ...
                if(IS_VAMPIRE(ch)) {
                    ...
                    ch->loc_hp[6] = 0;
                    if(char_up) {
                        send_to_char("You feel the strength of the kindred flow through your veins!\n\r", ch);
                    }
                    update_pos(ch);
                }
            }
    ...
    if(--pulse_gain_exp <= 0) {
        ...
        if(bootcount > 0) {
            ...
            default:
                ...
                break;
            case 600:
                strcpy(buf, "Reboot in ten minutes");
                break;
            case 300:
                strcpy(buf, "Reboot in five minutes");
                break;
            case 120:
                strcpy(buf, "Reboot in two minutes");
                break;
            case 60:
                strcpy(buf, "Reboot in one minute");
                break;
            case 30:
                strcpy(buf, "Reboot in thirty seconds");
                break;
            case 10:
                strcpy(buf, "Reboot in ten seconds");
                break;
            case 0:
                strcpy(buf, "Reboot NOW");
                break;
            }
            if(strlen(buf) > 2) {
                sysmes(buf);
                ...
                if(ch->pcdata->darkness > 0) {
                    ...
                    if(ch->pcdata->darkness < 1) {
                        if(IS_MORE(ch, MORE_DARKNESS)) {
                            ...
                            send_to_char("Your ability to call the Shroud of Night has returned.\n\r", ch);
                        }
                    }
                }
