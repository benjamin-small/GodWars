...
void do_clap(CHAR_DATA *ch, char *argument) {
    ...
    if(IS_NPC(ch)) {
        ...
        if(!IS_WEREWOLF(ch)) {
            no_dam = TRUE;
        } else if(get_tribe(ch, TRIBE_SHADOW_LORDS) < 2) {
            no_dam = TRUE;
        }
    }
    {
...
void do_wwpowers(CHAR_DATA *ch, char *argument) {
    ...
    if(get_breed(ch, BREED_LUPUS) > 0) {
        ...
        if(get_breed(ch, BREED_LUPUS) > 3) {
            strcat(buf, " Masssummons");
        }
        if(get_breed(ch, BREED_LUPUS) > 4) {
            strcat(buf, " Fleshbond");
        }
        strcat(buf, ".\n\r");
        send_to_char(buf, ch);
    }
    if(get_breed(ch, BREED_METIS) > 0) {
        ...
        if(get_breed(ch, BREED_METIS) > 3) {
            strcat(buf, " Witherlimb");
        }
        if(get_breed(ch, BREED_METIS) > 4) {
            strcat(buf, " Porcupine");
        }
        strcat(buf, ".\n\r");
        send_to_char(buf, ch);
    }
    ...
    if(get_auspice(ch, AUSPICE_PHILODOX) > 0) {
        strcpy(buf, "Philodox:");
        if(get_auspice(ch, AUSPICE_PHILODOX) > 2) {
            strcat(buf, " Scry");
        }
        if(get_auspice(ch, AUSPICE_PHILODOX) > 4) {
            strcat(buf, " Wallgranite");
        }
        strcat(buf, ".\n\r");
        send_to_char(buf, ch);
    }
    if(get_auspice(ch, AUSPICE_GALLIARD) > 0) {
        ...
        if(get_auspice(ch, AUSPICE_GALLIARD) > 2) {
            strcat(buf, " Beastwithin");
        }
        strcat(buf, ".\n\r");
        send_to_char(buf, ch);
    }
    if(get_auspice(ch, AUSPICE_AHROUN) > 0) {
        strcpy(buf, "Ahroun:");
        if(get_auspice(ch, AUSPICE_AHROUN) > 2) {
            strcat(buf, " Truefear");
        }
    ...
    if(get_tribe(ch, TRIBE_BLACK_FURIES) > 0) {
        ...
        if(get_auspice(ch, TRIBE_BLACK_FURIES) > 4) {
            strcat(buf, " Bodywrack");
        }
        strcat(buf, ".\n\r");
        send_to_char(buf, ch);
    }
    ...
    if(get_tribe(ch, TRIBE_FIANNA) > 0) {
        ...
        if(get_tribe(ch, TRIBE_FIANNA) > 2) {
            strcat(buf, " Illusionaryform");
        }
        if(get_tribe(ch, TRIBE_FIANNA) > 3) {
            strcat(buf, " Cloneobject");
        }
        strcat(buf, ".\n\r");
        send_to_char(buf, ch);
    }
    if(get_tribe(ch, TRIBE_GET_OF_FENRIS) > 0) {
        ...
        if(get_tribe(ch, TRIBE_GET_OF_FENRIS) > 2) {
            strcat(buf, " Scream");
        }
        strcat(buf, ".\n\r");
        send_to_char(buf, ch);
    ...
    if(get_tribe(ch, TRIBE_SHADOW_LORDS) > 0) {
        ...
        if(get_tribe(ch, TRIBE_SHADOW_LORDS) > 1) {
            strcat(buf, " Clap");
        }
        if(get_tribe(ch, TRIBE_SHADOW_LORDS) > 2) {
            strcat(buf, " Icy");
        }
        if(get_tribe(ch, TRIBE_SHADOW_LORDS) > 3) {
            strcat(buf, " Paralyze");
    ...
    if(get_tribe(ch, TRIBE_UKTENA) > 0) {
        ...
        if(get_tribe(ch, TRIBE_UKTENA) > 3) {
            strcat(buf, " Invisibility");
        }
        if(get_tribe(ch, TRIBE_UKTENA) > 4) {
            strcat(buf, " Fetishdoll");
        }
        strcat(buf, ".\n\r");
        send_to_char(buf, ch);
    }
    ...
    if(!any_powers) {
        send_to_char("You have no gifts.\n\r", ch);
    }
    send_to_char("--------------------------------------------------------------------------------\n\r", ch);
    return;
}
...
void do_gifts(CHAR_DATA *ch, char *argument) {
    ...
    if(arg[0] == '\0') {
        ...
        for(sn = BREED_HOMID; sn <= BREED_METIS; sn++) {
            ...
            if(ch->pcdata->cland[0] == sn) {
                strcpy(disc, upper_cap(disc));
            }
            if(ch->pcdata->breed[sn] != 0) {
                sprintf(buf, "%-16s: %-2d      ",
        ...
        if(!got_disc) {
            send_to_char("You currently have no gifts.\n\r", ch);
        }
        send_to_char("--------------------------------------------------------------------------------\n\r", ch);
    ...
    default:
        break;
    case 0:
        gift = ch->pcdata->breed[improve];
        break;
    case 1:
        gift = ch->pcdata->auspice[improve];
        break;
    case 2:
        gift = ch->pcdata->tribes[improve];
        break;
    }
    if(ch->pcdata->cland[gt] == improve) {
        ww_max = 5;
    }
    if(gift == 0) {
        send_to_char("You know of no such gift.\n\r", ch);
