...
SPEC_FUN *spec_lookup(const char *name) {
    ...
    if(!str_cmp(name, "spec_eater")) {
        return spec_eater;
    }
    if(!str_cmp(name, "spec_gremlin_original")) {
        return spec_gremlin_original;
    }
    if(!str_cmp(name, "spec_gremlin_born")) {
        return spec_gremlin_born;
    }
    if(!str_cmp(name, "spec_rogue")) {
        return spec_rogue;
    }
    if(!str_cmp(name, "spec_clan_guardian")) {
        return spec_clan_guardian;
    }
    if(!str_cmp(name, "spec_clan_torcalta")) {
        return spec_clan_torcalta;
    }
    if(!str_cmp(name, "spec_clan_spiritknights")) {
        return spec_clan_spiritknights;
    }
    if(!str_cmp(name, "spec_clan_werewolf")) {
        return spec_clan_werewolf;
    }
    if(!str_cmp(name, "spec_kavir_guardian")) {
        return spec_kavir_guardian;
    }
    if(!str_cmp(name, "spec_zombie_lord")) {
        return spec_zombie_lord;
    }
    if(!str_cmp(name, "spec_dog")) {
        return spec_dog;
    }
    return 0;
}
...
bool spec_gremlin_original(CHAR_DATA *ch) {
    ...
    if(number_percent() > 25) {
        ...
        if(victim != NULL) {
            ...
            if(speech == 3) {
                do_emote(ch, "looks around for any scraps of food.");
            }
            if(speech == 4) {
                if(!IS_NPC(victim)) {
...
bool spec_zombie_lord(CHAR_DATA *ch) {
    ...
    if(ch->spectype != ZOMBIE_TRACKING && ch->spectype != ZOMBIE_REST) {
        ...
        for(victim = char_list; victim != NULL; victim = victim->next) {
            ...
            if(IS_NPC(victim) && !str_cmp(victim->powertype, "zombie")) {
                continue;
            }
            if(IS_NPC(victim) && victim->pIndexData->vnum == 30011) {
                continue;
            }
            if(victim->in_room == ch->in_room) {
