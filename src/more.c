...
void do_baal(CHAR_DATA *ch, char *argument) {
    ...
    } else if(get_disc(ch, DISC_POTENCE) < 10) {
        potence = 1;
    } else {
        potence = 0;
    }
    ...
    } else if(get_disc(ch, DISC_CELERITY) < 10) {
        celerity = 1;
    } else {
        celerity = 0;
    }
    ...
    } else if(get_disc(ch, DISC_FORTITUDE) < 10) {
        fortitude = 1;
    } else {
        fortitude = 0;
    }
    ...
    ch->blood[0] -= blood;
    if((sn = skill_lookup("demonic possession")) < 0) {
        return;
    }
    af.type      = sn;
...
void do_scream(CHAR_DATA *ch, char *argument) {
    ...
    if(IS_NPC(ch)) {
        ...
        if(!IS_VAMPIRE(ch) && !IS_GHOUL(ch)) {
            no_dam = TRUE;
        } else if(get_disc(ch, DISC_MELPOMINEE) < 1) {
            no_dam = TRUE;
        }
    ...
    if(arg[0] == '\0') {
        ...
        for(victim = ch->in_room->people; victim; victim = victim_next) {
            ...
            if(!IS_NPC(victim) && (IS_VAMPIRE(victim) || IS_GHOUL(victim)) &&
                    get_disc(victim, DISC_MELPOMINEE) > 0) {
                continue;
            }
            if(is_safe(ch, victim)) {
                if(!IS_NPC(victim) && ch != victim && IS_MORE(victim, MORE_GUARDIAN)) {
...
void do_coterie(CHAR_DATA *ch, char *argument) {
    ...
    if(arg[0] == '\0' || !str_cmp(arg, "info")) {
        ...
        if(strlen(ch->pcdata->coterie) > 1 && strlen(ch->pcdata->leader) > 1 &&
            ...
            for(gch = char_list; gch != NULL; gch = gch->next) {
                ...
                if(strlen(gch->pcdata->coterie) < 2) {
                    continue;
                }
                if(strlen(gch->pcdata->cot_time) < 2) {
                    continue;
                }
                if(strlen(gch->pcdata->leader) < 2) {
                    continue;
                }
                if(str_cmp(ch->pcdata->leader, gch->pcdata->leader)) {
                    continue;
                }
                if(str_cmp(ch->pcdata->cot_time, gch->pcdata->cot_time)) {
                    continue;
                }
        ...
        for(level = COT_BANNED; level <= COT_EXPERT; level++) {
            for(value = DISC_ANIMALISM; value <= DISC_VICISSITUDE; value++) {
                ...
                case DISC_THANATOSIS:
                    ...
                    break;
                case DISC_THAUMATURGY:
                    disc = POW_THAUMATURGY;
                    break;
                case DISC_VICISSITUDE:
                    disc = POW_VICISSITUDE;
                    break;
                }
    ...
    if(!str_cmp(arg, "rules")) {
        ...
        if(!str_cmp(arg2, "clans")) {
             ...
             * capitalised.
             */
            else if(!str_cmp(arg3, "assamite")) {
                strcpy(arg3, "Assamite");
            } else if(!str_cmp(arg3, "assamites")) {
                strcpy(arg3, "Assamite");
            } else if(!str_cmp(arg3, "baali")) {
                strcpy(arg3, "Baali");
            } else if(!str_cmp(arg3, "brujah")) {
                strcpy(arg3, "Brujah");
            } else if(!str_cmp(arg3, "caitiff")) {
                strcpy(arg3, "Caitiff");
            } else if(!str_cmp(arg3, "daughters")) {
                strcpy(arg3, "Daughters");
            } else if(!str_cmp(arg3, "gangrel")) {
                strcpy(arg3, "Gangrel");
            } else if(!str_cmp(arg3, "giovanni")) {
                strcpy(arg3, "Giovanni");
            } else if(!str_cmp(arg3, "lasombra")) {
                strcpy(arg3, "Lasombra");
            } else if(!str_cmp(arg3, "malkavian")) {
                strcpy(arg3, "Malkavian");
            } else if(!str_cmp(arg3, "nosferatu")) {
                strcpy(arg3, "Nosferatu");
            } else if(!str_cmp(arg3, "panders")) {
                strcpy(arg3, "Panders");
            } else if(!str_cmp(arg3, "ravnos")) {
                strcpy(arg3, "Ravnos");
            } else if(!str_cmp(arg3, "samedi")) {
                strcpy(arg3, "Samedi");
            } else if(!str_cmp(arg3, "serpents")) {
                strcpy(arg3, "Serpents");
            } else if(!str_cmp(arg3, "setite")) {
                strcpy(arg3, "Setite");
            } else if(!str_cmp(arg3, "setites")) {
                strcpy(arg3, "Setite");
            } else if(!str_cmp(arg3, "toreador")) {
                strcpy(arg3, "Toreador");
            } else if(!str_cmp(arg3, "tremere")) {
                strcpy(arg3, "Tremere");
            } else if(!str_cmp(arg3, "tzimisce")) {
                strcpy(arg3, "Tzimisce");
            } else if(!str_cmp(arg3, "ventrue")) {
                strcpy(arg3, "Ventrue");
        ...
        /* Specify which sects are permitted in the coterie. */
            ...
            if(!str_cmp(arg3, "all")) {
                ...
                strcpy(arg3, "Anarch");
            } else if(!str_cmp(arg3, "camarilla")) {
                strcpy(arg3, "Camarilla");
        ...
        /* Specify what generation new members should be. */
            ...
            int min_gen = 13;
            if(ch->pcdata->cot_min[COT_EXPERT] > 0) {
                min_gen = 7;
            }
            if(arg3[0] == '\0') {
        ...
        /* Specify discipline requirements of new members. */
            ...
            if(!str_cmp(arg2, "banned")) {
                level = COT_BANNED;
            } else if(!str_cmp(arg2, "basic")) {
                level = COT_BASIC;
            } else if(!str_cmp(arg2, "intermediate")) {
                level = COT_INTERMEDIATE;
            } else if(!str_cmp(arg2, "advanced")) {
                level = COT_ADVANCED;
            } else {
                level = COT_EXPERT;
            }
...
void do_ctalk(CHAR_DATA *ch, char *argument) {
    ...
    for(d = descriptor_list; d != NULL; d = d->next) {
        ...
        if(strlen(victim->pcdata->leader) < 2) {
            continue;
        }
        if(str_cmp(ch->pcdata->coterie, victim->pcdata->coterie)) {
            continue;
...
void ritual_ward(CHAR_DATA *ch, int direction, int ward) {
    ...
    if((pexit = ch->in_room->exit[rev]) == NULL) {
        return;
    }
    if(IS_SET(pexit->exit_info, ward)) {
        return;
    }
    switch(rev) {
    default:
...
void show_disc(CHAR_DATA *ch) {
    ...
    if(IS_SET(ch->pcdata->cot_min[COT_EXTRA], COT_ADV_STANCE)) {
        if((IS_MORE(ch, MORE_LEADER) && !IS_MORE(ch, MORE_CHAMPION)) ||
            ...
            send_to_char("You must have an advanced stance to join.\n\r", ch);
        }
...
void do_unburden(CHAR_DATA *ch, char *argument) {
    ...
    if(victim->beast > victim->truebeast && victim->beast > 1) {
        ...
        ch->pcdata->obeah = 30;
        if(victim->beast < victim->truebeast) {
            victim->beast = victim->truebeast;
        }
