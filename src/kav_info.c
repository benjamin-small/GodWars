...
void skillstance(CHAR_DATA *ch, CHAR_DATA *victim) {
    ...
    if(stance < 1 || stance > 15) {
        return;
    }
    if(IS_NPC(victim) && (stance < 1 || stance > 10)) {
        return;
    }
    ...
    if(stance == STANCE_VIPER) {
        ...
        if(strlen(victim->pcdata->stancename) < 2) {
            return;
        }
...
void show_spell(CHAR_DATA *ch, int dtype) {
    ...
    } else if(ch->spl[dtype] >= 240 && IS_MAGE(ch)) {
...
void do_totalscore(CHAR_DATA *ch, char *argument) {
    ...
    if(k_score < 1) {
        sprintf(buf, "Exp from kills / time played: +0.\n\r");
    } else {
    ...
    send_to_char(buf, ch);
    p_score = ch->stance[1];
    if(ch->stance[2]  > ch->stance[1]) {
        p_score = ch->stance[2];
    }
    if(ch->stance[3]  > ch->stance[2]) {
        p_score = ch->stance[3];
    }
    if(ch->stance[4]  > ch->stance[3]) {
        p_score = ch->stance[4];
    }
    if(ch->stance[5]  > ch->stance[4]) {
        p_score = ch->stance[5];
    }
    p_score *= 0.25;
    t_score += p_score;
    ...
    send_to_char(buf, ch);
    p_score = ch->wpn[0];
    if(ch->wpn[1]  > ch->wpn[0]) {
        p_score = ch->wpn[1];
    }
    if(ch->wpn[2]  > ch->wpn[1]) {
        p_score = ch->wpn[2];
    }
    if(ch->wpn[3]  > ch->wpn[2]) {
        p_score = ch->wpn[3];
    }
    if(ch->wpn[4]  > ch->wpn[3]) {
        p_score = ch->wpn[4];
    }
    if(ch->wpn[5]  > ch->wpn[4]) {
        p_score = ch->wpn[5];
    }
    if(ch->wpn[6]  > ch->wpn[5]) {
        p_score = ch->wpn[6];
    }
    if(ch->wpn[7]  > ch->wpn[6]) {
        p_score = ch->wpn[7];
    }
    if(ch->wpn[8]  > ch->wpn[7]) {
        p_score = ch->wpn[8];
    }
    if(ch->wpn[9]  > ch->wpn[8]) {
        p_score = ch->wpn[9];
    }
    if(ch->wpn[10] > ch->wpn[9]) {
        p_score = ch->wpn[10];
    }
    if(ch->wpn[11] > ch->wpn[10]) {
        p_score = ch->wpn[11];
    }
    if(ch->wpn[12] > ch->wpn[11]) {
        p_score = ch->wpn[12];
    }
    p_score *= 0.25;
    t_score += p_score;
    ...
    send_to_char(buf, ch);
    p_score = ch->spl[0];
    if(ch->spl[1]  > ch->spl[0]) {
        p_score = ch->spl[1];
    }
    if(ch->spl[2]  > ch->spl[1]) {
        p_score = ch->spl[2];
    }
    if(ch->spl[3]  > ch->spl[2]) {
        p_score = ch->spl[3];
    }
    if(ch->spl[4]  > ch->spl[3]) {
        p_score = ch->spl[4];
    }
    p_score *= 0.25;
    t_score += p_score;
    ...
    if(p_score > 0 && ch->pcdata->score[SCORE_NUM_QUEST] > 0) {
        ...
        sprintf(buf, "Average qp earned per completed quest: +0.\n\r");
    }
    send_to_char(buf, ch);
    t_score += p_score;
    ...
    if(!str_cmp(first_place, ch->name)) {
        lowest = score_1;
    } else if(!str_cmp(second_place, ch->name)) {
        lowest = score_2;
    } else if(!str_cmp(third_place, ch->name)) {
        lowest = score_3;
    } else if(t_score < lowest) {
        return;
    }
    if(lowest == score_1) {
            ...
            if(in_obj->carried_by == ch) {
                ...
                if(IS_SET(obj->spectype, SITEM_DEMONIC)) {
                    if(obj->specpower == ARM_GREY) {
                        max_val = 100;
                    } else if(obj->specpower == ARM_PURPLE) {
                        max_val = 250;
